...
export type IProductPopup = IBlockData<
  {
...
export const ProductPopup = createCustomBlock<IProductPopup>({
  name: 'Product Popup',
  type: CustomBlocksType.PRODUCT_POPUP,
  validParentType: [BasicType.PAGE, AdvancedType.WRAPPER, BasicType.WRAPPER],
  create: payload => {
    const defaultData: IProductPopup = {
      type: CustomBlocksType.PRODUCT_POPUP,
      data: {
        value: {
  ...
  render: ({ data, idx, mode, context, dataSource }) => {
    console.log(data, idx, mode, context, dataSource);
